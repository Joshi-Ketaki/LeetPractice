...
public:
    ...
    void enqueue(int element) {
        ...
        //wait will (unlock+wait, wakeup, take lock again. so it does not hold lock
        //if condition is not met. and we do not have to reacquire lock if condition is met.)
        cv.wait(lock, [&](){return q.size() < cap;});
        q.push(element);
        // this would do the unlock as well. Check again.
        cv.notify_all();
    }
